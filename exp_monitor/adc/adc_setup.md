...
## Software Setup
  ...
  * Therefore, sensors and equipment that are read out over analog should be sub-classes of the Arduino class and use its `super().measure` method.
  * In order for its sub-classes to be able to make independent `measure` calls, the analog values of the Arduino are stored locally for the duration of the cycle. These are the values that are then accessed by `Arduino` sub-classes during their `measure` calls, which means that they strictly speaking don't initiate a measurement, but in this way the consistent interface of the `Sensor` class is maintained.
