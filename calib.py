...
class Calibrator:
    ...
    def __init__(self):
        ...
        if not os.path.isfile(os.getcwd() + '/calib.p'):
            self.calib_meas = []
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 8, 28), (3.9e-9, 3.17)))
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 8, 30), (2.1e-9, 2.85)))
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 10, 46), (1.3e-9, 2.61)))
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 11, 52), (7.5e-10, 2.31)))
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 12, 16), (6.9e-10, 2.27)))
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 13, 26), (8.2e-10, 2.36)))
            self.calib_meas.append((datetime.datetime(2021, 4, 27, 15, 55), (1.2e-9, 2.59)))
            self.calib_meas.append((datetime.datetime(2021, 4, 28, 8, 10), (4.2e-9, 3.20)))
            self.calib_meas.append((datetime.datetime(2021, 4, 28, 8, 17), (2.0e-9, 2.84)))
            self.calib_meas.append((datetime.datetime(2021, 4, 28, 9, 13), (1.5e-9, 2.68)))
            self.calib_meas.append((datetime.datetime(2021, 4, 28, 9, 54), (1.4e-9, 2.66)))
            self.calib_meas.append((datetime.datetime(2021, 4, 29, 7, 59), (7.8e-11, 1.10)))
    ...
    def calibrate(self, show=True):
        ...
        # Perform fit:
        self.popt, self.pcov = curve_fit(fit_fctn, self.data_x, self.data_y, p0=initial_guess, bounds=bounds)
        ...
        if show is True:
            ...
            ax.legend()
            ax.set_title(r'Fit results: $P(V) = %.2f \times 10^{%.2f \times V - %.2f} + %.2f$' % (self.popt[0], self.popt[1], self.popt[2], self.popt[3]))
            plt.show()
